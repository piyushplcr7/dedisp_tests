...
* functional test applications with generated data [README](bin/test/README.md)
...
We have tested the three implementations on NVIDIA Titan RTX with CUDA 11. Our TDD implementation is a factor 1.5x to 2x faster than dedisp (depending on `#DMs`). FDD is again up to 20% faster than TDD, for `#DMs > 512`, and is expected to scale better with advancements in GPU architecture. Where dedisp and TDD are memory bound, FDD is compute bound.
...
TDD is functionally the same as the original dedisp and produces the same result with both libdedisp testcases and original filterbank data.
...
We have tested our approach with Fourier-domain dedispersion. In FDD, the shifts of time series at individual observing frequencies are replaced by phase rotations, this is enabled by Fourier transforming each time series from the time-domain to the spin frequency-domain. Phase rotations are implemented as multiplication by phasors. This transformation of the dedispersion algorithm requires much less I/O and enables more efficient computations. The final results are transferred back to the time-domain, or can be kept in the Fourier-domain for further processing. We will describe the algorithm and implementation in more detail in our publication.

FDD is functionally similar to the original dedisp. The testcases of libdedisp show small absolute differences with the original, but the same DMs are found in both generated test data and original filterbank data. Also on further analysis of the output we find the same results with FDD as with the original implementation. If you are interested in this implementation please test it with your pipeline. You can contact the authors for more information.
...
The default implementation and alternative implementations are listed in the two sections below.

### CPU
These are methods from `dedisp::FDDCPUPlan`
- `FDDCPUPlan dedisperse_reference()`
  - Might be enabled at runtime through setting environment variable: `USE_REFERENCE=1`
...
- `FDDCPUPlan dedisperse_optimized()`
  ...
  - This is what you get when running `testfdd` with environment variable: `USE_CPU=1`
- `FDDCPUPlan dedisperse_segmented_reference()`
  - Might be enabled at runtime through setting environment variables: `USE_REFERENCE=1` and `USE_SEGMENTED=1`
...
- `FDDCPUPlan dedisperse_segmented_optimized()`
  - Might be enabled at runtime through setting environment variable: `USE_SEGMENTED=1`
