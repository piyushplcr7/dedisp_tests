...
usage(void)
{
  ...
  fprintf(stderr,"               1 double value between 0 and oo\n");
  fprintf(stderr,"               default: `0'\n");
  fprintf(stderr,"      -fftout: Output the FFT coefficients instead of time series data.\n");
  fprintf(stderr,"     -multout: Output individual files containing time series or fft coefficients for different DM values.\n");
...
parseCmdline(int argc, char **argv)
{
  ...
  for(i=1, cmd.argc=1; i<argc; i++) {
    ...
    if( 0==strcmp("-fftout", argv[i]) ) {
      cmd.fftoutP = 1;
      continue;
    }

    if( 0==strcmp("-multout", argv[i]) ) {
      cmd.multoutP = 1;
      continue;
    }

    if( 0==strcmp("-cleanout", argv[i]) ) {
      cmd.cleanoutP = 1;
      continue;
    }
