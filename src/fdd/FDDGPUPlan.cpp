...
void FDDGPUPlan::setOutputParams(
        ...
        int w,
        bool barycenter) {
  // The MPI/segmented overlap-add path requires the dirty layout
...
#define TRACE(...) do { fprintf(tlog, __VA_ARGS__); fflush(tlog); } while (0)
  ...
  std::thread mpi_thread = std::thread([&]() {
    ...
    if (barycenter_) {
      ...
      global_input_idx_end = global_input_idx_start + ( (mpi_rank > 0) ? nsamps : nsamps - max_delay);

      // Go over the global inForOut to find important positions
      auto inForOut = container_->getInForOut();
      int out_idx_global;
      for (out_idx_global = 0 ; out_idx_global < inForOut.size() ; ++out_idx_global) {
