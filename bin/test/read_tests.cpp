...
void getDataFromRow(int fd, unsigned char *rawdata, float *data_scl,
                    float *data_offs, float *data_wts, int subint) {
  // Move to dat_wts col
  lseek(fd, 12364, SEEK_CUR);

  // read dat_wts
  read(fd, data_wts, 12288);
  // read data_offs
  read(fd, data_offs, 49152);
  // read data_scl
  read(fd, data_scl, 49152);
  // read data
  read(fd, rawdata, 122880000);
...
void getDataFromRow_buffered(int fd, unsigned char *rawdata, float *data_scl,
                    float *data_offs, float *data_wts, int subint, unsigned char* buffer) {
  // Reading a complete row in the buffer
  read(fd, buffer, 123002956);
  
  // Buffer position for data_wts
  unsigned char* buffer_pos = buffer + 12364;
  memcpy((void*)data_wts, (void*)buffer_pos, 12288);
  buffer_pos += 12288;
  memcpy((void*)data_offs, (void*)(buffer_pos), 49152);
  buffer_pos += 49152;
  memcpy((void*)data_scl, (void*) (buffer_pos), 49152);
  buffer_pos += 49152;
  memcpy((void*)rawdata, (void*) (buffer_pos), 122880000);
